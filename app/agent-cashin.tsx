      ...
      {step === 'amount' && (
        <View style={styles.section}>
          ...
          {parsedAmount > 0 && (
            <View style={[styles.infoRow, { backgroundColor: colors.surface }]}>
              <View style={styles.infoItem}>
                <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>Your Commission</Text>
          ...
          <View style={styles.bottomActions}>
            <TouchableOpacity
              style={[styles.actionBtn, { backgroundColor: validAmount && canAfford ? colors.primary : colors.border }]}
              onPress={handleConfirm}
              disabled={!validAmount || !canAfford}
            >
              <Text style={[styles.actionBtnText, { color: colors.white }]}>Continue</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
        ...
        <View style={styles.section}>
          <View style={[styles.detailsCard, { backgroundColor: colors.surface }]}>
            <View style={styles.detailRow}>
              ...
              <Text style={[styles.detailValue, { color: colors.text }]}>{formatPhone(normalizePhone(phone))}</Text>
            </View>
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>Deposit Amount</Text>
              <Text style={[styles.detailAmount, { color: colors.text }]}>{formatCurrency(parsedAmount)}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>Your Wallet Debit</Text>
              <Text style={[styles.detailValue, { color: colors.error }]}>-{formatCurrency(parsedAmount)}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>Your Commission</Text>
              <Text style={[styles.detailValue, { color: '#22c55e' }]}>+{formatCurrency(estimatedCommission)}</Text>
            </View>
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <View style={styles.detailRow}>
              <Text style={[styles.detailLabel, { color: colors.text, fontWeight: '700' }]}>Net Cost to You</Text>
              <Text style={[styles.detailAmount, { color: colors.text, fontWeight: '800' }]}>
                {formatCurrency(parsedAmount - estimatedCommission)}
              </Text>
            </View>
          </View>

          <View style={[styles.reminderBox, { backgroundColor: '#f59e0b15', borderColor: '#f59e0b30' }]}>
            <Ionicons name="alert-circle" size={18} color="#f59e0b" />
            <Text style={[styles.reminderText, { color: '#f59e0b' }]}>
              Make sure you have collected {formatCurrency(parsedAmount)} cash from the customer before confirming.
            </Text>
          </View>
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                <Text style={[styles.actionBtnText, { color: colors.white }]}>Confirm — Cash Collected</Text>
              </>
            )}
          </TouchableOpacity>

          <TouchableOpacity style={styles.cancelBtn} onPress={() => setStep('amount')}>
            <Text style={[styles.cancelText, { color: colors.textSecondary }]}>Go Back</Text>
          </TouchableOpacity>
        </View>
      )}
